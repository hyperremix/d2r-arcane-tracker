...
function simulatePrefix(nextU32, cfg, names){
  ...
  for(let h=0; h<HOURS; h++){
    ...
    if(chosen!==expectedId) return matched; // stop at first mismatch
    matched++;
...
function run(){
  ...
  const preWarmups=[0,1,2,4,8,16,32,64,128,256,512,1024,2048,4096];
        ...
        for(const preWarmup of preWarmups){
          for(const perHour of perHourSkips){
            for(const postHour of postHourSkips){
              ...
              if(prefix>best.prefix){ best={prefix,label:pr.label,cfg:{useBounded,windowSize,preWarmup,perHourSkips:perHour,postHourSkips:postHour}}; if(prefix===HOURS){ console.log(JSON.stringify({success:true,best},null,2)); return; } }
              // small pruning: if prefix<3 for a while, continue
            }
          }
        }
      }
