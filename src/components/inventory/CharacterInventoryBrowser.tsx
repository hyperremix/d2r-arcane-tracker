...
function getCharacterFilterParts(characterFilterValue: string): {
  characterId?: string;
  characterName?: string;
} {
  if (characterFilterValue === 'all') {
    return {};
  }

  if (characterFilterValue.startsWith('name:')) {
    return {
      characterName: characterFilterValue.slice('name:'.length),
    };
  }

  return {
    characterId: characterFilterValue,
  };
}
...
export function CharacterInventoryBrowser() {
  ...
  const [characterFilterValue, setCharacterFilterValue] = useState('all');
  ...
  const loadInventorySearch = useCallback(async (): Promise<void> => {
    const characterFilter = getCharacterFilterParts(characterFilterValue);
    ...
    setIsLoading(false);
  }, [characterFilterValue, locationContext, searchText]);
  ...
  const characterOptions = useMemo(() => {
    ...
    for (const snapshot of inventoryResponse?.inventory.snapshots ?? []) {
      const value = snapshot.characterId ?? `name:${snapshot.characterName}`;
      options.set(value, snapshot.characterName);
    }
        ...
        <Card>
          ...
          <CardContent className="grid grid-cols-1 gap-3 md:grid-cols-5">
            ...
            <Select
              ...
              onValueChange={(value) => setCharacterFilterValue(value ?? 'all')}
            >
              <SelectTrigger>
