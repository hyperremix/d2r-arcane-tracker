...
2. **terrorzone-schedule.csv** (from https://d2emu.com/tz-sp)
   - 938 hourly entries
   - Date range: 2025-09-27 to 2025-12-06
   - Starts at hour 23,398 from seed start time
...
- **Near-uniform distribution**: All 36 zones appear 21-30 times in 938 entries
- **No consecutive duplicates**: Same zone never appears twice in a row (100% enforced)
- **Zone frequency variation**: ±20% from expected uniform distribution

### Repeat Distance Analysis
```
Average repeat distance: 35.65 hours ≈ 36 (number of zones!)
Min repeat distance: 3 hours
Max repeat distance: 119 hours

Most common distances:
  30 hours: 30 occurrences
  24 hours: 26 occurrences
  26 hours: 24 occurrences
  28 hours: 24 occurrences
```
...
**Best accuracy: 5% (essentially random guessing)**
...
## Why a Bag System is Likely

**Evidence:**
1. ✓ Average repeat distance = ~36 hours (exactly the number of zones)
2. ✓ Near-uniform distribution over time
3. ✓ No consecutive duplicates (can be enforced in bag draw)
4. ✓ Minimum distance of 3 (allows rare early repeats)
5. ✓ No fixed repeating sequences (each shuffle is different)
6. ✓ Most common distances cluster around 24-34 hours

**Possible Implementation:**
```
1. Create a bag with all 36 zones
2. Shuffle the bag using a PRNG seeded with (base_seed + bag_number)
3. Draw zones from the bag sequentially
4. If drawn zone equals previous zone, draw the next one
5. When bag is empty (or nearly empty), create and shuffle a new bag
6. Increment bag_number for next shuffle
```

**Why it doesn't match:** The exact PRNG and seeding strategy are unknown.
...
**Best Accuracy Achieved**: 5% (random baseline)
