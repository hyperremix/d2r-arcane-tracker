...
describe('When vault item database operations are executed', () => {
  ...
  describe('If character IDs are not available for vault rows', () => {
    it('Then search can filter by source character name', () => {
      // Arrange
      upsertVaultItemByFingerprint(ctx, {
        fingerprint: 'fp-char-sorc',
        itemName: 'Shako',
        quality: 'unique',
        ethereal: false,
        rawItemJson: '{}',
      ...
      upsertVaultItemByFingerprint(ctx, {
        fingerprint: 'fp-char-barb',
        itemName: 'Arreat',
        quality: 'unique',
        ethereal: false,
        rawItemJson: '{}',
        sourceCharacterName: 'Barb',
        sourceFileType: 'd2s',
        locationContext: 'inventory',
      });
