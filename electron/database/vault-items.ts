...
function addCharacterFilterClause(
  ...
  if (filter.characterName) {
    clauses.push("lower(COALESCE(vi.source_character_name, '')) = ?");
    params.push(filter.characterName.trim().toLowerCase());
  }
}
...
export function searchVaultItems(
  ...
  addCharacterFilterClause(filter, clauses, params);
